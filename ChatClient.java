...
public class ChatClient extends JFrame implements ActionListener, Runnable {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == txtname || e.getSource() == btnconn) { // 대화명 입력 후
																	// 접속
			...
			try {
				soc = new Socket("192.168.0.17", 7777);
		...
		} else if (e.getSource() == txtsend || e.getSource() == btnok) { // 메세지
																			// 전송
			try {
				if (radio1.isSelected()) { // 귓속말
					String name = list.getSelectedItem();
					out.write(("/s" + name + "-" + txtsend.getText() + "\n").getBytes("euc-kr"));
					txtarea.append(name + "님에게 귓속말이 전송되었습니다.\n");
				} else { // 일반 메세지
	...
	public void run() {
		while (true) {
			try {
				String msg = in.readLine(); // 서버로부터 자료 수신
				if (msg == null || msg.equals(""))
					return;

				if (msg.charAt(0) == '/') {
					if (msg.charAt(1) == 'c') { // 대화명(최초접속)
